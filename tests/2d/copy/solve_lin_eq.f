...
!                     IJK_GL= funijk_gl(i,j,k)
!	             ie = ie + 1 
!        write(80+myPE,*)ie,IJK,i,j,k,IJK_GL,KM_OF(IJK) - IJK, IM_OF(IJK) - IJK, & 
!        JM_OF(IJK) - IJK, JP_OF(IJK) - IJK, IP_OF(IJK) - IJK, KP_OF(IJK) - IJK
!
!        write(90+myPE,*)ie,IJK,IJK_GL,A_M(IJK,-3,M), A_M(IJK,-1,M), A_M(IJK,-2,M), A_M(IJK,0,M), & 
!        A_M(IJK,2,M), A_M(IJK,1,M), A_M(IJK,3,M), B_M(IJK,M),VAR(IJK) 
!		  end do 
!		end do 
!	     end do 
...
!        key="KEY_EXIT"
!        call xmlint(KEY_EXIT, key)
!        write(*,*) "KEY_EXIT", VNAME, KEY_EXIT, NIT
!!        if(KEY_EXIT.eq.1.and.VNAME.eq."EPp") then
!         if(KEY_EXIT.eq.1.and.LEQ_METHOD(1).eq.2.and.NIT.eq.13) then
!!        if(KEY_EXIT.eq.1.and.LEQ_METHOD(6).eq.2) then
!
!
!!        DO IJK = ijkstart3, ijkend3
!!            write(700+myPE,*) IJK, VAR(IJK) !myPE, ijk, IM_OF(IJK) - IJK, IP_OF(IJK) - IJK
!!        ENDDO
!
!!         filename ="lineqn_M"
!!!         call WRITE_ABVAR1(A_M(:,:,M), B_M(:,M),VAR,filename)
!!         call WRITE_matvec(A_M(:,:,M), B_M(:,M),VAR,filename)
!!         write(820,*) VNAME, method, time3
!
!
!
!!$omp parallel do default(shared) private(ijk,ijk_gl,i,j,k,isk)
!            do k = kstart2,kend2
!               do i = istart2,iend2
!                  do j = jstart2,jend2
!                     IJK = funijk(i,j,k)
...
!        JM_OF(IJK) - IJK, JP_OF(IJK) - IJK, IP_OF(IJK) - IJK, KP_OF(IJK) - IJK
!      
!        write(90+myPE,*)IJK-isk,IJK_GL-isk,A_M(IJK,-3,M), A_M(IJK,-1,M), A_M(IJK,-2,M), A_M(IJK,0,M), & 
!        A_M(IJK,2,M), A_M(IJK,1,M), A_M(IJK,3,M), B_M(IJK,M)
!                  enddo
!               enddo
!            enddo
!
...
!!                     !B_M(IJK,M)    = IJK_GL/100.00
!!                     B_M(IJK,M)    = A_M(IJK,-3,M)+A_M(IJK,-2,M)+A_M(IJK,-1,M)+A_M(IJK,0,M)+A_M(IJK,1,M)+A_M(IJK,2,M)+A_M(IJK,3,M); 
!!                    if(myPE.eq.0.and.IJK_GL.eq.JMAX2+1) then 
!!                    write(*,*) "IJK_GL = 1", IJK_GL
!!		    B_M(IJK,M) =       A_M(IJK,0,M) +A_M(IJK,1,M)+A_M(IJK,2,M)+A_M(IJK,3,M)
!!	    	    end if 
!!		    if(myPE.eq.0.and.IJK_GL.eq.JMAX2+2) then
!!                    write(*,*) "IJK_GL = 1", IJK_GL
!!                    B_M(IJK,M) =  A_M(IJK,-2,M)+ A_M(IJK,0,M) +A_M(IJK,1,M)+A_M(IJK,2,M)+A_M(IJK,3,M)
!!                    end if
!!
!!		    if(myPE.gt.0.and.IJK_GL.eq.286) then
!!		    write(*,*) "IJK_GL = ijkmax1", IJK_GL
!!	            B_M(IJK,M) = A_M(IJK,-3,M)+A_M(IJK,-2,M)+A_M(IJK,-1,M)+A_M(IJK,0,M)
!!		    end if 
!!		    if(myPE.gt.0.and.IJK_GL.eq.285) then
!!                    write(*,*) "IJK_GL = ijkmax1", IJK_GL
!!                    B_M(IJK,M) = A_M(IJK,-3,M)+A_M(IJK,-2,M)+A_M(IJK,-1,M)+A_M(IJK,0,M) + A_M(IJK,2,M)
!!                    end if
!!
!!        write(80+myPE,*)IJK,i,j,k,IJK_GL,KM_OF(IJK) - IJK, IM_OF(IJK) - IJK, & 
!!        JM_OF(IJK) - IJK, JP_OF(IJK) - IJK, IP_OF(IJK) - IJK, KP_OF(IJK) - IJK
