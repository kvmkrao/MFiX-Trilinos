...
!	dimred = funijk(iend2,jend2,kend2) - funijk(istart2,jstart2,kend2) - JMAX2 + 1
	dimred = funijk(iend1,jend1,kend1) - funijk(istart1,jstart1,kstart1)  + 1
...
!         if(abs(A_M(IJK, 2)).lt.1) 
          ...
          Anew(ie,7) =  A_M(IJK,3)
	  Bn(ie)     =  B_M(IJK)
	  locgl(ie)  =  IJK_GL - funijk(istart1,jstart1,kstart1) + 1
	  locijk(ie) =  IJK
...
!	end if 
       ...
       if(myPE.eq.(numPEs-1)) ntrows  = FUNIJK_GL(iend1,jend1,kend1) - & 
                                        FUNIJK(istart1,jstart1,kstart1) + 1 
        call MPI_BCAST(ntrows,1,MPI_INTEGER,(numPEs-1), & 
        MPI_COMM_WORLD,mpierr) 


	write(*,*) "istart", myPE, istart1,istart2,istart3,istart4
	write(*,*) "iend", myPE, iend1,  iend2,  iend3,  iend4
!	stop 
...
!	call FWrapper_ext(Bn, xn, pos, Anew,locgl,cols,dimred, ntrows)
	call FWrapper_ext(Bn, xn, pos, Anew,locgl,cols,ie, ijkmax1, jmax2)
