...
/*
    ...
    else if (glon[i] == NumGlobalElements-2) {
      NumNz[i] = 4;
      }
*/
...
//   if(Comm.MyPID() == 0) nend = nend - 4 *102 +1;
...
/*
  if ( glon[i] ==0) {
      Indices[0] = glon[i]+ aloc[i][3];
      Indices[1] = glon[i]+ aloc[i][4];
      Indices[2] = glon[i];
      NumEntries = 3;
      Values[0]  = Ao[i][4];
      Values[1]  = Ao[i][5];
      ...
      Indices[0] = glon[i] + aloc[i][2];
      Indices[1] = glon[i] + aloc[i][3];
      Indices[2] = glon[i] + aloc[i][4];
      Indices[3] = glon[i];
      NumEntries = 4;
      Values[0]  = Ao[i][2];
      Values[1]  = Ao[i][4];
      ...
      Indices[0] = glon[i] + aloc[i][1];
      Indices[1] = glon[i] + aloc[i][2];
      Indices[2] = glon[i] + aloc[i][3];
      Indices[3] = glon[i];
      NumEntries = 4;
      Values[0]  = Ao[i][1];
      Values[1]  = Ao[i][2];
...
//    lclerr = A.InsertGlobalValues(glon[i], NumEntries, Values, Indices);
    lclerr = A.SumIntoGlobalValues(gid, NumEntries, Values, Indices);
    // Put in the diagonal entry
//    glbrow = glon[i];
//    two = Ao[i][2];
//    lclerr = A->InsertGlobalValues(glon[i], 1, &two,&glbrow);
     }
}

/*
      NumEntries = 0 ;
      if (RowLess2 >=0 &&  aloc[i][1] != 0 )
      {
...
//       A.insertGlobalValues(gblRow, 1, &Ao[i][1], &RowLess2);
       ...
       if (RowLess1 >=0  &&  aloc[i][2] != 0)
       {
       Values[NumEntries]  = Ao[i][2];
       Indices[NumEntries] = RowLess1;
       NumEntries          = NumEntries + 1 ;
       }
       if (RowPlus1 < NumGlobalElements  && aloc[i][3] !=0) 
       {
      ...
      NumEntries          = NumEntries + 1 ;
       ...
       if (RowPlus2 < NumGlobalElements    && aloc[i][4] !=0)
       {
      ...
      NumEntries          = NumEntries + 1 ;
        ...
        std::cout << " Inserted a row # " <<  gid << " entries" << NumEntries <<  std::endl;
  }
*/
