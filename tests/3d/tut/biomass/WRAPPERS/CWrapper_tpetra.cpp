...
//  typedef typename CrsMatrixType::ST ST;
//  typedef typename CrsMatrixType::LO LO;
//  typedef typename CrsMatrixType::GO GO;
...
//    Teuchos::RCP<MueLu::TpetraOperator<ST,LO,GO,node_type> >  
       RCP<const mtoperator > mueLuPreconditioner ( A_ptr, optionsFile); 
//     MueLu::CreateTpetraPreconditioner (const Teuchos::RCP< Tpetra::CrsMatrix<ST, LO,GO, node_type> >& A_ptr, optionsFile);
//  std::string optionsFile = "mueluOptions.xml";
...
//  solverParams->set ("Explicit Residual Scaling", "None");
  ...
  typedef Belos::SolverFactory<ST, multivector_type, op_type> belos_factory_type;
  typedef Belos::SolverManager<ST, multivector_type, op_type> solver_type;
...
//   commented : Murali
 typedef Belos::LinearProblem<ST, multivector_type, op_type> problem_type;
