...
extern "C"
{
  double* CPPWrapper(double** A, double* b, double* x_vec, double** aloc, int* globn, int rows, int cols, int tnrows, int maxj);
  void cwrapper_(double* br, double* xr, double* alc, double* A, int *globn, int *nAx, int *nAy, int *nrows, int *mnodesj, char Str[7]) // note extra underscore to keep linker happy
  {
int n = *nAx;
int m = *nAy;
...
//   std::cout << *mnodesj << std::endl; 

//  std::cout << alc[0] << alc[1]<< alc[2]  << alc[3] << alc[4] << alc[5] << std::endl; 
  CPPWrapper(Amat,br,xr,loc,globn,m,n,*nrows, *mnodesj);
...
double* CPPWrapper(double** Ao, double* bo, double* x_vec ,double** aloc,int* globn, int rows, int cols,int tnrows, int maxj) // A[n*m]x[m] = b[n]
  {
...
//  Values[0] = -1.0; Values[1] = -1.0;
  ...
  std::cout << " Cpp wrapper: maxj" << maxj << std::endl; 
  
  if(Comm.MyPID() != Comm.NumProc() -1) nend = nend - 2 *maxj;
...
//    std::cout << gid << " "<< glon[i] << std::endl; 
	...
	NumEntries = 0 ; 
      if (RowLess2 >=0 &&  abs(aloc[i][1]) != 0 && Ao[i][1] != 0 )
      {
...
//       A.insertGlobalValues(gblRow, 1, &Ao[i][1], &RowLess2);
       ...
       if (RowLess1 >=0  &&  abs(aloc[i][2]) != 0 && Ao[i][2] != 0)
       {
...
//       if (RowPlus1 < NumGlobalElements &&  RowPlus1 !=GlobalRow) //aloc[i][3] != 0.0)
       if (RowPlus1 < NumGlobalElements &&  abs(aloc[i][3]) != 0 && Ao[i][4] !=0)
       {
...
//       A.insertGlobalValues(gblRow, 1, &Ao[i][4], &RowPlus1);
       ...
       if (RowPlus2 < NumGlobalElements &&  abs(aloc[i][4]) != 0 && Ao[i][5] !=0)
       {
...
//  RHS.SumIntoGlobalValues(NumMyElements, glon, bo, 0);
  RHS.SumIntoGlobalValues(nend, glon, bo, 0);
  RHS.GlobalAssemble();
