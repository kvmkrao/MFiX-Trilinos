...
!         if(abs(A_M(IJK,-2)).lt.1) 
	  ...
	  if(IJK_GL.lt.2*JMAX2)  pos(ie,2) = 0 
	  if(IJK_GL.gt.ijkmax1)  pos(ie,5) = 0  
	  ...
	  if(pos(ie,5).eq.0) Anew(ie,4) = Anew(ie,4) + Anew(ie,6)
        ...
        write(80+myPE,888)ie,i,j,k,IJK_GL-JMAX2,pos(ie,1),pos(ie,2),pos(ie,3),pos(ie,4), & 
        pos(ie,5),pos(ie,6)
...
!	if(myPE.eq.0) ie = ie - JMAX2*2 
	...
	write(*,*) ijkmax1, ijkmax2, ijkmax3, ijkmax4, ijkend3, ijkend4
