...
//   Epetra_FECrsMatrix A(Copy,Map,7);
 ...
 int lclerr = 0;
  ...
  double elapsed_time, total_flops, MFLOPs,tstop,tstart;
...
//      RHS->ReplaceGlobalValue(glbrow, 0, 1.0);
 ...
 RHS.SetFlopCounter(counter);

 Epetra_LinearProblem problem(&A, &LHS, &RHS);
 AztecOO Solver(problem);

   std::string inputFile="input_param.xml";
   std::string PrecondType,precname,lsolver;
   bool prec;
   ...
   lsolver     = myParams->get<std::string>("Solver");
   prec        = myParams->get<bool>("Precond");
   PrecondType = myParams->get<std::string>("PrecondType");
...
/* 
 if(prec) { 
  if(PrecondType=="incmfact") {
     Teuchos::ParameterList List;
     List.set("relaxation: type", "Jacobi");
     List.set("relaxation: sweeps", 2);
     List.set("relaxation: damping parameter", 0.9);

     Ifpack_PointRelaxation ifPrec(&A);

     ifPrec.SetParameters(List);
//     Prec.SetParameters(&ifpackpar);
     ifPrec.Initialize(); // perform the symbolic factorization.
     ifPrec.Compute(); // perform the numeric factorization.
   //  Solver.SetPrecOperator(&ifPrec);
  }
  else {
   std::string mlpar="setpre.xml";
   ML_Epetra::MultiLevelPreconditioner* MLPrec = new ML_Epetra::MultiLevelPreconditioner(A, mlpar, true);
...
//   Solver.SetPrecOperator(MLPrec);
   ...
   elapsed_time = timer.ElapsedTime() - tstart ;
   MFLOPs = total_flops/elapsed_time/1000000.0;

   
   if (Comm.MyPID() == 0) cout << "Total operations   " << total_flops << endl;
   if (Comm.MyPID() == 0) cout << "MFlops per second  " << MFLOPs/elapsed_time << endl ; 
