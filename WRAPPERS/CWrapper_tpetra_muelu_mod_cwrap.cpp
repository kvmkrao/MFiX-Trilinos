...
*/
  ...
  // CPPWrapper(Amat, br, xr,loc, globn, m, n, *nn, *maxit, *tol);
   CPPWrapper(A, br, xr,alc, globn, m, n, *nn, *maxit, *tol);
...
//  std::clock_t c_start1 = std::clock();
  ...
  for(lcR = 0; lcR < static_cast<LO> (numMyElements);  ++lcR) {
    ...
    RowLess3 = gblRow + aloc[i1];
    RowLess2 = gblRow + aloc[i2];
    RowLess1 = gblRow + aloc[i3];
    RowPlus1 = gblRow + aloc[i4];
    RowPlus2 = gblRow + aloc[i5];
    RowPlus3 = gblRow + aloc[i6];
...
*/
    ...
    if ((RowLess3 >=0 )&&  abs(aloc[i1]) != 0.0 ){
      Values[NumEntries]  = Ao[i1]; 
      Indices[NumEntries] = RowLess3; 
      NumEntries          = NumEntries + 1 ; 
    }

//      if (RowLess2 >=0 &&  RowLess2 !=GlobalRow) //aloc[i][1] != 0.0)
    if ((RowLess2 >=0 ) && abs(aloc[i2]) != 0) { 
      Values[NumEntries]  = Ao[i2];
      Indices[NumEntries] = RowLess2;
      NumEntries          = NumEntries + 1 ;
    }
  
    if ((RowLess1 >=0 ) && abs(aloc[i3]) != 0)  {
      Values[NumEntries]  = Ao[i3];
      Indices[NumEntries] = RowLess1;
      NumEntries          = NumEntries + 1 ;
    }
  
//       if (RowPlus1 < NumGlobalElements &&  RowPlus1 !=GlobalRow) //aloc[i][3] != 0.0)
    if ((RowPlus1 < numGlobalElements) &&  abs(aloc[i4]) != 0)   {
       Values[NumEntries]  = Ao[i5];
       Indices[NumEntries] = RowPlus1;
       NumEntries          = NumEntries + 1 ;
    }
    
   if ((RowPlus2 < numGlobalElements) &&  abs(aloc[i5]) != 0)       {
      Values[NumEntries]  = Ao[i6];
      Indices[NumEntries] = RowPlus2;
      NumEntries          = NumEntries + 1 ;
   }
   
   if ((RowPlus3 < numGlobalElements) && abs(aloc[i6]) != 0)   {
      Values[NumEntries]  = Ao[i7];
      Indices[NumEntries] = RowPlus3;
      NumEntries          = NumEntries + 1 ;
      }

      Values[NumEntries]  = Ao[i4];
