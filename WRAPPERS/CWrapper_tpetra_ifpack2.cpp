...
RCP<prec_type> ifpack2Preconditioner = factory.create<crs_matrix_type> (prectype, A);
...
//    Problem->setRightPrec (mueLuPreconditioner);
      Problem->setRightPrec( ifpack2Preconditioner );
//    The left-preconditioned case has not yet been implemented.  Please use right preconditioning for now.  If you need to use left preconditioning, please contact the Belos developers.  Left preconditioning is more interesting in BiCGStab because whether it works depends on the initial guess (e.g., an initial guess of all zeros might NOT work
    }
